...
void EventStorage::init(){
    ...
    // TODO - Generated method body
    //get sumo data
    net = new NetInputHandle("C:\\omnet\\sumoData\\map.net.xml");
    trafficmap = net->getMap();
    sumotr = new SumotrInputHandle("C:\\omnet\\sumoData\\map.sumo.tr", trafficmap);
...
//        }
    ...
    if(tnow - monitorTime >= monitor_interval ){
        ...
        //***********send monitor message************************** //每隔5s进行监控全局信息
        map<int, OperatorPlacementManager*>::iterator opmIt = opm.begin();
        while(opmIt != opm.end()){
...
void EventStorage::replacement_decision(){//进行重部署机制判断哪 1.是否超过响应时间阈值，2. 是否决策 decision是一样的 一样的就不重部署
    int nodeIndex = intensiveNodeID;
